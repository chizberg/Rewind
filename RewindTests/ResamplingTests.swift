...
struct ResamplingTests {
  ...
  @Test func bicubicShrinkMatchesPillow() {
    let plane = Plane<UInt8>(
      size: PlaneSize(width: 5, height: 3),
      values: [
        0, 200, 50, 255, 100,
    ...
    let resized = plane.bicubicResized(target: PlaneSize(width: 3, height: 2))

    #expect(resized.size == PlaneSize(width: 3, height: 2))
    #expect(resized.values == [68, 133, 154, 124, 95, 125])
  }

  @Test func bicubicUpscaleClampsOvershoot() {
    let plane = Plane<UInt8>(
    ...
    let resized = plane.bicubicResized(target: PlaneSize(width: 5, height: 4))

    #expect(resized.size == PlaneSize(width: 5, height: 4))
